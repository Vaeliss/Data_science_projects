...
plot(streaming_BMs,oos="y",mode="i",xlim=c(1,20),ylim=c(0,15000),xaxt="n")
axis(1, at= seq(1, length(dati_streaming$x[32:48])+3,1), labels = c(dati_streaming$year[32:48],2021,2022,2023))
...
plot(c(dati_streaming$x[32:48]), type="l",xaxt="n",xlab = "Year",ylab = "Adjusted Value",ylim=c(0,15000))
...
# UCRCD cd vs digital
ill.down.rate <- c(0.08,0.15,0.2,0.23,0.25,0.23,0.21,0.18,0.15,rep(0.15,13))
#ill.down.rate <- c(0.15,0.2,0.23,0.25,0.26,0.26,0.25,0.25,0.25,0.2,0.15,0.12,0.1,0.08,rep(0.05,7))
plot(dati_cd$year[27:48],ill.down.rate,main="Estimated illegal download ratio",xlab="Year",ylab="Ratio")
ill.down.estimate <- dati_cd$x[27:48] *ill.down.rate / 0.8
#ill.down.estimate <- c(dati_cd$x[27]*0.1/0.9,ill.down.estimate)
ill.down.estimate
plot(dati_cd$year[27:48], ill.down.estimate, main="Estimated lost revenue due to large scale piracy",xlab="Year",ylab="Revenue")
digital
...
ucrcd_cd_downloads <- UCRCD(cd, downloads, 16) 
summary(ucrcd_cd_downloads)
coef(ucrcd_cd_downloads)

# new bass model downloads
tsdisplay(downloads)
es_downloads_BMs <- BASS.standard(downloads, display=F, ous=100, prelimestimates = c(50000,0.01,0.1)) 
summary(es_downloads_BMs) # all highly significant
predicted_es_downloads_BMs <- predict(es_downloads_BMs, newx=1:70)

plot(downloads, type="l",xaxt="n",xlab = "Year",ylab = "Yearly revenue in dollars (milions adjusted)",main="Downloads sales (piracy effect)",xlim=c(1,24),ylim=c(0,6000))
axis(1, at = seq(1,length(dati_downloads$year[27:48])+2,1),labels = c(dati_downloads$year[27:48],2021,2022))
lines(predicted_es_downloads_BMs, col=2)

#ucrcd downloads/streaming
