...
rock <- c("album rock","alternative rock","art rock","australian alternative rock",
          ...
          "irish rock","modern rock","rock-and-roll","soft rock","yacht rock","celtic punk",
          "pop punk","punk")
...
hist(dati$Popularity, col="orange", main="Songs popularity", xlab="Popularity",freq = F)
...
hist(dati$Speechiness,nclass=10)

# Set train and test
set.seed(42)
train = sample (1:nrow(dati), 0.8*nrow(dati))
...
# Stepwise Regression
m2 <- step(m1, direction="both") # procedura selezione variabili
summary(m2)
par(mfrow=c(2,2))
plot(m2)
par(mfrow=c(1,1))
...
dev.lm
AIC(m2)
mean(abs(p.lm-dati.test$Popularity))
...
par(mfrow=c(1,1))
plot(g2,se=T,terms="genere")
#plot(g2, se=T, ask=T)
unique(dati.train$genere)
...
mean(abs(p.gam-dati.test$Popularity))
...
# 4 Boosting - combination of previous models, QUESTO
boost.songs=gbm(Popularity ~ .,data=dati.train, 
...
# absolute error instead of quadratic
boost.songs=gbm(Popularity ~ .,data=dati.train, 
                distribution="laplace",n.trees=5000, interaction.depth=4, shrinkage=0.01)
...
yhat.boost=predict(boost.songs ,newdata=dati.test,n.trees=1:5000)
err = apply(yhat.boost, 2, function(pred) mean(abs(dati.test$Popularity-pred)))
plot(err, type="l")
